...
- **Reload** — Kill the debug PID (from **`/tmp/dartsnut_dbg.pid`**), restart the same Python command (re-runs **`ensure_app_venv`** on uv firmware when deps/version stamp changed). Does **not** start **`dartsnut_python.service`**.
