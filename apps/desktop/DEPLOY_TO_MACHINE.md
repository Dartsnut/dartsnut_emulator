...
- **Run** — Pack the workspace (`tar`), upload, extract on device, **`systemctl stop dartsnut_python.service`**, then start **`/home/rpi/dartsnut_rpi/venv0/bin/python -u /home/rpi/dartsnut_rpi/apps/<id>/main.py`** with **`WorkingDirectory=/home/rpi/dartsnut_rpi/apps/<id>`** (same layout as **`widget_lifecycle`** subprocesses), inside **`sudo bash -c 'cd … && exec …'`**, with **`PYTHONUNBUFFERED=1`** and **`DARTSNUT_LOG_LEVEL=INFO`** so **`/tmp/dartsnut_deploy.log`** fills immediately. Logs also stream in the UI via **`tail -f`**. This is **not** the repo-root **`dartsnut_rpi/main.py`** used by **`dartsnut_python.service`**.
