...
export function buildPostIntakeCreatorUserPrompt(
  ...
  if (options?.forceBuildAfterIntake || postIntakeCreatorStatesBuildIntent(originalUserPrompt)) {
    ...
    const buildNowLead = openEnded
      ? "**Build now (mandatory):** The user asked for an open-ended surprise — **pick exactly one concrete widget/game concept** (one short line in assistant text if needed), then implement per **Success criteria**: load **`karpathy-guidelines`**, **`creator-incremental`**, **`conf-contract`**, **`pydartsnut-core`** with **`get_dartsnut_skill`** (parallel OK), then tool-first scaffold and iteration. **Do not** run a second brainstorm after skill results return."
      : "**Build now (mandatory):** The user's first message already states what to create. Implement per **Success criteria**: load **`karpathy-guidelines`**, **`creator-incremental`**, **`conf-contract`**, **`pydartsnut-core`** first, then scaffold with tools (`conf.json`, verify, stub `main.py`, verify, iterate). Tool-first — no long plan before the first edit unless the task is unusually complex. Load other skills only when that step needs them.";
